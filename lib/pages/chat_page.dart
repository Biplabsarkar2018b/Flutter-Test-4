...
import 'package:test4/models/message_chat.dart';
import 'package:test4/pages/full_photo_page.dart';
...
class _ChatPageState extends State<ChatPage> {
  ...
  void readLocal(){
    ...
    if (currUserId.compareTo(peerId) > 0) {
      groupChatId = '$currUserId-$peerId';
    } else {
      groupChatId = '$peerId-$currUserId';
    }
  ...
  Future uploadFile() async {
   ...
   void onSendMessage(String content, int type) {
    if (content.trim().isNotEmpty) {
      ...
      chatProvider.sendMessage(content, type, groupChatId, currUserId, widget.arguments.peerId);
  ...
  Widget buildItem(int index,DocumentSnapshot? document){
    if (document != null) {
      ...
      if (messageChat.idFrom == currUserId) {
        // Right (my message)
        return Row(
          children: <Widget>[
  ...
  bool isLastMessageLeft(int index) {
    if ((index > 0 && listMessage[index - 1].get(FirestoreConstants.idFrom) == currUserId) || index == 0) {
      return true;
    } else {
      return false;
    }
